...
DATASET="ca-GrQc"
MODEL="ckpt/model.pt"
TRIALS=100  # Optimized value

# Create output directories
mkdir -p logs plots/cluster/{greedy,mcts,beam} results/{greedy,mcts,beam}
...
python -m subgraph_mining.decoder \
    ...
    | tee logs/greedy.log
mv plots/cluster/*.html plots/cluster/greedy/
mv results/out-patterns* results/greedy/
...
python -m subgraph_mining.decoder \
    ...
    | tee logs/mcts.log
mv plots/cluster/*.html plots/cluster/mcts/
mv results/out-patterns* results/mcts/

# 3. Beam Search (Specialized Parameters)
...
python -m subgraph_mining.decoder \
    ...
    --max_pattern_size=5 \
    | tee logs/beam.log
mv plots/cluster/*.html plots/cluster/beam/
mv results/out-patterns* results/beam/
```
